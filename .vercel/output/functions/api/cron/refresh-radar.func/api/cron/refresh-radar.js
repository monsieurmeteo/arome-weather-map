...
const RAIN_PALETTE = [[0.0, 0.1, 0, 0, 0, 0], [0.1, 0.15, 4, 4, 132, 200], [0.15, 0.2, 4, 20, 156, 210], [0.2, 0.3, 4, 40, 180, 215], [0.3, 0.4, 4, 64, 204, 220], [0.4, 0.5, 4, 88, 220, 220], [0.5, 0.6, 4, 112, 236, 225], [0.6, 0.9, 4, 140, 252, 225], [0.9, 1.2, 20, 168, 220, 225], [1.2, 1.5, 8, 180, 8, 230], [1.5, 2.0, 24, 200, 24, 230], [2.0, 2.7, 60, 216, 60, 230], [2.7, 3.7, 100, 232, 100, 230], [3.7, 4.8, 180, 232, 20, 235], [4.8, 6.0, 232, 232, 4, 235], [6.0, 8.0, 252, 216, 4, 235], [8.0, 12.0, 252, 176, 4, 240], [12.0, 16.0, 252, 132, 4, 240], [16.0, 27.0, 252, 88, 4, 240], [27.0, 37.0, 252, 36, 4, 245], [37.0, 48.0, 220, 4, 4, 245], [48.0, 65.0, 188, 4, 4, 245], [65.0, 85.0, 176, 4, 100, 250], [85.0, 120.0, 160, 4, 160, 250], [120.0, 150.0, 176, 44, 200, 250], [150.0, 210.0, 200, 80, 220, 250], [210.0, 278.0, 220, 120, 240, 250], [278.0, 370.0, 236, 160, 248, 250], [370.0, 486.0, 248, 200, 252, 250], [486.0, 9999, 255, 240, 255, 255]];

// ============================================================
// Supabase helpers
// ============================================================
function getSupabase() {
  return (0, _supabaseJs.createClient)(SUPABASE_URL, SUPABASE_KEY, {
    auth: {
...
async function getExistingManifest(supabase) {
  try {
    const {
      data
    } = supabase.storage.from(BUCKET).getPublicUrl('manifest.json');
...
function extractH5Files(gzipData) {
  ...
  const decompressed = (0, _zlib.gunzipSync)(gzipData);
  const h5Files = {};
  let offset = 0;
...
async function h5ToPngBuffer(h5wasm, h5Data, timestampStr) {
  const {
    ...
    undetect,
    bounds
  } = await parseH5Radar(h5wasm, h5Data);
  ...
  const pngBuffer = _pngjs.PNG.sync.write(png, {
    colorType: 6,
    filterType: 4
  });
...
async function handler(req, res) {
  ...
  try {
    ...
    const existingTimestamps = new Set((existingManifest?.frames || []).map(f => f.timestamp));
    ...
    for (const [tsStr, h5Data] of sortedFiles) {
      ...
      const {
        ...
        bounds,
        size
      } = await h5ToPngBuffer(h5wasm, h5Data, tsStr);
      latestBounds = bounds;
      ...
      const {
        error
      } = await supabase.storage.from(BUCKET).upload(filename, pngBuffer, {
    ...
    const manifest = {
      ...
      bounds: latestBounds,
      leaflet_bounds: Object.keys(latestBounds).length ? [[latestBounds.LL_lat || 38.14, latestBounds.UL_lon || -9.965], [latestBounds.UL_lat || 53.67, latestBounds.UR_lon || 17.56]] : [],
      frame_count: finalFrames.length,
      frames: finalFrames
    };
    const {
      error: mError
    } = await supabase.storage.from(BUCKET).upload('manifest.json', Buffer.from(JSON.stringify(manifest, null, 2)), {
    ...
    return res.status(200).json({
      success: true,
      new_frames: newCount,
      total_frames: finalFrames.length,
...
//# sourceMappingURL=refresh-radar.js.map